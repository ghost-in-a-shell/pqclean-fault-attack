...
class GaussMatrix:
    ...
    def verify_solution(self, solution):
        for d in self.matrix:
            result = 0
            for r in list(map(lambda x,y:0 if None == y else x*y, d, solution)):
                result += r
...
def random_test(times):
    ...
    for i in range(times):
        ...
        solution = map(lambda x : random.randint(0, mod - 1), [xc for xc in range(col)])

        matrix = []
        for y in range(row):
            array = list(map(lambda x : random.randint(0, mod), [xc for xc in range(col)]))

            t = 0
            for j in list(map(lambda x,y:0 if None == y else x*y, array, solution)):
                t += j
            array.append(t % mod)
...
def static_test():
    ...
    for i in range(len(matrix)):
        t = 0
        for j in list(map(lambda x,y:0 if None == y else x*y, matrix[i], solution)):
            t += j
